...
class ENC_Conv(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, bias=False, reduce=True, blind=True):
        super().__init__()
        self.reduce = reduce
        self.conv = Conv(in_channels, out_channels, bias=bias, blind=blind)
        if reduce:
...
class Blind_UNet_small(nn.Module): #student A, 0.12m
...
if __name__ == '__main__':
    model = Blind_UNet_small(n_channels=3, n_output=3, bias=False, blind=False)
