...
def main():
    ...
    parser.add_argument('--launcher', choices=['none', 'pytorch'], default='none',
                        help='job launcher')
    parser.add_argument('--local_rank', type=int, default=0) #不可为local_rank
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0 and current_step>0:
                ...
                for val_data in val_loader:
                    ...
                    corruption = misc_utils.tensor2img(visuals['corruption'][0])  # --> uint8 0-255
                    gt = misc_utils.tensor2img(visuals['gt'][0])  # --> uint8 0-255

                    single_psnr = batch_utils.batch_PSNR(model.pred[0].unsqueeze(0).detach().cpu()/255, model.gt[0].unsqueeze(0).detach().cpu()/255, 1.)
                ...
                # log psnr and ssim
                best_psnr_pred = max(psnr_pred, best_psnr_pred)
                best_iter = current_step if psnr_pred == best_psnr_pred else best_iter
                logger.info('# Validation of Pred # PSNR: {:.4e}. # SSIM: {:.4e}. || Best PSNR: {:.4e} at iter: {:d}.'.format(
                    psnr_pred, ssim_pred, best_psnr_pred, best_iter))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}. ssim: {:.4e}. || Best PSNR: {:.4e} at iter: {:d}.'.format(
                    epoch, current_step, psnr_pred, ssim_pred, best_psnr_pred, best_iter))
                # tensorboard logger
                if opt['use_tb_logger'] and 'debug' not in opt['name']:
                    tb_logger.add_scalar('psnr_pred', psnr_pred, current_step)
                    tb_logger.add_scalar('ssim_pred', ssim_pred, current_step)
                # save best model
                if psnr_pred == best_psnr_pred:
                    logger.info('Saving best model and training states.')
                    model.save("best")
                    model.save_training_state(epoch, current_step, opt["model"], 'best')
